...
public interface IdentityClient {
    ...
    ResponseEntity<TenantResponse> getTenantById(@RequestHeader("Authorization") String authorization,
                                                 @PathVariable String tenantId);

    @RequestMapping(value = "/api/identity/v1/tenants/by-name/{tenantName}", method = RequestMethod.GET)
    ResponseEntity<TenantResponse> getTenantByName(@RequestHeader("Authorization") String authorization,
