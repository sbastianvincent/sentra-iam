...
public class AuthControllerTests {
    ...
    void setup() {
        ...
        authController = new AuthController(authenticationService, config);
    }
    ...
    void testOpenIdConfiguration () {
        Mockito.when(config.getOpenidConfigurationUri()).thenReturn("http://localhost:8080/auth/v1/keys/");

        String tenantId = "test-tenant";
        Map<String, Object> response = authController.openIdConfiguration(tenantId);

        Mockito.verify(config, Mockito.times(2)).getOpenidConfigurationUri();
